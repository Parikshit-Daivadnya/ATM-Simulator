...
//       int randomNum = ran.nextInt(1000);  // guaranteed to be 0–999

        JLabel title1Label= new JLabel("Application Form");
        title1Label.setFont(new Font("Oswald", Font.BOLD, 32));
        title1Label.setBounds(260,10,550,80);
        add(title1Label);
    ...
    public void actionPerformed(ActionEvent e) 
    {
       ...
       // String formNo= randomNum ;
        ...
        try{
            ...
            {
                Conn conn = new Conn(); // Ensure class name matches
                Conn c= new Conn();
                String query= "insert into signup1(form_No,First_Name,Last_Name,Father_Name,Date,Gender,Email,Marital_Status,Address,City,PinCode,State) values ('"+formNo+"', '"+fname+"', '"+lname+"','"+fathername+"', '"+dob+"', '"+gender+"', '"+email+"', '"+marriedstatus+"','"+addr+"', '"+city+"','"+pincode+"','"+state+"')" ;
                PreparedStatement ps = conn.c.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
                ps.executeUpdate();
                
                ResultSet rs = ps.getGeneratedKeys();
                if (rs.next()) {
                int formId = rs.getInt(1);
                String formNumber = "ATM" + String.format("%04d", formId);

                // Step 2: Update form_number
                String updateQuery = "UPDATE signup1 SET form_No = ? WHERE form_id = ?";
                PreparedStatement ps2 = conn.c.prepareStatement(updateQuery);
                ps2.setString(1, formNumber);
                ps2.setInt(2, formId);
                ps2.executeUpdate();

                JOptionPane.showMessageDialog(null, "Your Application Form No. is " + formNumber);
